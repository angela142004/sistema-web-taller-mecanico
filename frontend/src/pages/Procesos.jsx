    ...
    useEffect(() => {
        ...
        if (e.key === "ArrowLeft") setIndex((i) => (i - 1 + procesos.length) % procesos.length);
        };
    ...
    const prev = () => setIndex((i) => (i - 1 + procesos.length) % procesos.length);

    const paso = procesos[index];

    return (
        <div className="relative min-h-screen w-full flex flex-col items-center justify-center overflow-hidden text-white bg-black">
        {/* 🔹 Fondo global */}
        <div
        ...
        <div className="relative z-10 w-full max-w-7xl px-4 sm:px-6 py-12 flex flex-col gap-8">
            {/* Header */}
            ...
            <div className="md:flex-1 p-6 md:p-12 flex flex-col justify-center gap-6 bg-black/60 backdrop-blur-sm rounded-b-3xl md:rounded-r-3xl">
                ...
                <div>
                    <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-purple-300 drop-shadow-md">{paso.titulo}</h2>
                    <p className="text-sm text-gray-400 mt-1">{paso.subtitulo}</p>
                </div>
                </div>
                <p className="text-base sm:text-lg text-gray-200 leading-relaxed max-w-full md:max-w-3xl">{paso.descripcion}</p>
            ...
            <div className="flex gap-3 overflow-x-auto py-1 scroll-pl-4 md:scroll-pl-0 snap-x snap-mandatory">
                {procesos.map((p, i) => (
                <button
                    key={p.id}
                    onClick={() => setIndex(i)}
